...
import Comment from 'connectors/mongoose/models/comment'
import Entry from 'connectors/mongoose/models/entry'
import Numeral from 'connectors/mongoose/models/numeral'
...
function createComment(comment) {
  ...
  ['type', 'entryId', 'numeralId', 'content', 'see', ].map(prop => {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (updates.comments.length !== 0) {
      updates.comments.forEach(comment => {
        comment.numeralId = numeral.id;
        comment.type = "NUMERAL"
